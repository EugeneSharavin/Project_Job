...
Процедура ПроверитьНаличиеДублейПодарочныхСертификатовВДокументе(Документ, ИмяТабличнойЧасти, Отказ) Экспорт
	...
	Для Каждого Строка Из Документ[ИмяТабличнойЧасти] Цикл
		...
		Иначе
			ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТабличнойЧасти, Строка.НомерСтроки, "ПодарочныйСертификат");
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 подарочный сертификат указан повторно!'"), Строка.НомерСтроки), Документ, ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция ПолучитьМоментПолученияСведений(Ссылка, Дата) Экспорт
	Возврат ?(Ссылка.Пустая(), ?(ЗначениеЗаполнено(Дата), КонецДня(Дата), КонецДня(ТекущаяДатаСеанса())), Новый Граница(Ссылка.МоментВремени(), ВидГраницы.Исключая));
КонецФункции

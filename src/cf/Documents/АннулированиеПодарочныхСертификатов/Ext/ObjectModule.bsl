...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.СертификатАктивирован И ВыборкаДетальныеЗаписи.ДействуетДо >= Дата Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 подарочный сертификат еще действует и срок действия не истек!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;
		
		Если ВыборкаДетальныеЗаписи.СертификатАннулирован Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 подарочный сертификат уже аннулирован!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;

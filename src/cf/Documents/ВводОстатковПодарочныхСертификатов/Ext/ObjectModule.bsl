...
Процедура ПроверитьОстаткиСтоимостиПоСертификатам(Отказ)
	РеквизитыВидаПодарочныхСертификатов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПодарочныхСертификатов, "Валюта, НоминальнаяСтоимость");
	
	СтатусАктивирован = Перечисления.СтатусыПодарочныхСертификатов.Активирован;
	СтатусЧастичноИспользован = Перечисления.СтатусыПодарочныхСертификатов.ЧастичноИспользован;
	
	Для Каждого Строка Из ПодарочныеСертификаты Цикл
		ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ПодарочныеСертификаты", Строка.НомерСтроки, "ОстатокСуммы");
		...
		Если Строка.Статус = СтатусАктивирован Тогда
			Если Строка.ОстатокСуммы <> РеквизитыВидаПодарочныхСертификатов.НоминальнаяСтоимость Тогда
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Строка %1: в случае активации подарочного сертификата остаток неиспользованной суммы должен быть строго равен номинальной стоимости (%2 %3)!'"), 
					Строка.НомерСтроки, РеквизитыВидаПодарочныхСертификатов.НоминальнаяСтоимость, РеквизитыВидаПодарочныхСертификатов.Валюта), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
			КонецЕсли;		
		...
		Иначе
			Если Строка.ОстатокСуммы > 0 Тогда
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Строка %1: в случае выпуска, полного погашения, возврата или аннуляции остаток неиспользованной суммы должен быть равен нулю!'"), Строка.НомерСтроки), 
					ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбработкаПроведения(Отказ, Режим)
    ...
    ПроверитьОстаткиСтоимостиПоСертификатам(Отказ);

...
Процедура СформироватьДвиженияПоПодарочнымСертификатам(Отказ, Режим)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ОплатаПодарочнымиСертификатами", ВыборкаДетальныеЗаписи.НомерСтроки, "ПодарочныйСертификат");
		
		Если НЕ ВыборкаДетальныеЗаписи.СертификатАктивирован Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 подарочный сертификат не был активирован ранее (или уже полностью использован)!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;
		
		Если ВыборкаДетальныеЗаписи.ДействуетДо < Дата Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 истек срок действия подарочного сертификата!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;
		
		Если ВыборкаДетальныеЗаписи.НеиспользованныйОстатокСтоимости = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 номинальная стоимость подарочного сертификата полностью погашена!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, ПутьКТабличнойЧасти, , Отказ);
		ИначеЕсли ВыборкаДетальныеЗаписи.СуммаОплаты > ВыборкаДетальныеЗаписи.НеиспользованныйОстатокСтоимости Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 сумма оплаты превышает доступный к оплате остаток по сертификату!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, "ОплатаПодарочнымиСертификатами[" + Формат(ВыборкаДетальныеЗаписи.НомерСтроки - 1, "ЧГ=") + "].СуммаОплаты", , Отказ);
		КонецЕсли;
...
//Добавил Шаравин Евгений 24.05.2025 по проектной работе курса "Архитектор 1С" (УЦ ОТУС)

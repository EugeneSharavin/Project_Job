...
Процедура СформироватьДвиженияПоПодарочнымСертификатам(Отказ, Режим)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ДействуетДо < Дата Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'В строке %1 истек срок действия подарочного сертификата!'"), ВыборкаДетальныеЗаписи.НомерСтроки), ЭтотОбъект, "ПодарочныеСертификаты[" + Формат(ВыборкаДетальныеЗаписи.НомерСтроки - 1, "ЧГ=") + "].ПодарочныйСертификат", , Отказ);
		КонецЕсли;

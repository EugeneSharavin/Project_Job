...
Процедура ЗаполнитьВалютуВОплатеПодарочнымиСертификатами()
	...
	//{Добавил Шаравин Евгений 23.05.2025 по проектной работе курса "Архитектор 1С" (УЦ ОТУС)
...
Процедура ОплатаПодарочнымиСертификатамиПодарочныйСертификатПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ПодарочныйСертификат) Тогда
		...
		ТекущиеДанные.СуммаОплаты = Макс(Мин(Объект.Товары.Итог("Сумма") - Объект.ОплатаПодарочнымиСертификатами.Итог("СуммаОплаты"), ТекущиеДанные.ОстатокСуммы), 0);
	Иначе
...
Процедура ОплатаПодарочнымиСертификатамиПодарочныйСертификатНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ПодарочныеСертификаты.ФормаВыбора", Новый Структура("ТолькоАктивированные, Ссылка, Дата, Валюта", Истина, Объект.Ссылка, Объект.Дата, Объект.Валюта), Элемент);
КонецПроцедуры
...
Процедура Подбор(Команда)
	...
	ВыбранныеПодарочныеСертификаты = ПодарочныеСертификатыКлиент.ПолучитьПодарочныеСертификатыДокумента(Объект.ОплатаПодарочнымиСертификатами);
	ОткрытьФорму("Справочник.ПодарочныеСертификаты.Форма.ФормаВыбора", Новый Структура("МножественныйВыбор, ТолькоАктивированные, Ссылка, Дата, Валюта, ВыбранныеПодарочныеСертификаты", Истина, Истина, Объект.Ссылка, Объект.Дата, Объект.Валюта, ВыбранныеПодарочныеСертификаты), , ,  , , ОписаниеОповещения);
КонецПроцедуры
